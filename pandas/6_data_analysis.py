...
#        - 만약 데이터 프레임에 문자열이나 결측치 등 중앙값을 계산할 수 없는 데이터가 많으면 오류 발생 또는 잘못된 결과가 나옴 (=> 전처리 후 진행!)

# df.groupby(컬럼명)[집계대상컬럼] : 그룹화 기준 컬럼 지정 후 집계 수행 대상 컬럼 선택

# * transform() : groupby로 그룹화 된 데이터에 집계 함수 적용 시 사용
...
print('* === agg === *')
print(region_agg)
# => 지역 별로 판매 금액의 평균, 총합, 개수를 각각 출력(표시)
...
#  - 최빈값 : 가장 자주 나오는 값. mode() -> 반환되는 값은 Series

print(f'판매 수량의 최빈값 : {df["Units_Sold"].mode()}')
print(df['Units_Sold'].mode())

# => 최빈값의 경우 여러 개의 데이터가 결과로 표시되어, 값 자체를 사용할 떄 첫번째 데이터를 선택!
...
# df['Sales_Grade'] = np.where(df['Sales_Amount'] > df['Sales_Amount'].mean(), 'High', 'Low')
